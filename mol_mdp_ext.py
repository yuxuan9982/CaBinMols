...
class MolMDPExtended(MolMDP):
    ...
    def build_translation_table(self):
        ...
        for blockidx in range(len(self.block_mols[:-self.core_num])):
            ...
            atom_map = {}
            for j in range(len(self.block_mols[:-self.core_num])):
    ...
    def parents(self, mol=None):
        ...
        for rblockidx in blocks_degree_1:
            if mol.blockidxs[rblockidx]>=len(self.block_mols[:-self.core_num]):
                continue #如果是骨架分子，不能删
            new_mol = mol.copy()
            ...
            # Remove the block
            mask = np.ones(len(new_mol.blockidxs), dtype=np.bool if hasattr(np, 'bool') else np.bool_)
            mask[rblockidx] = 0
...
# def test_mdp_parent():
#     mdp = MolMDPExtended("./data/blocks.json")
...
def test_mdp_parent():
    mdp = MolMDPExtended("./data/blocks_sub.json", "./data/blocks_core.json")
    mdp.build_translation_table()
    import tqdm
    rng = np.random.RandomState(142)
    nblocks = mdp.num_blocks
    ...
    for i in tqdm.tqdm(range(10000)):
        ...
        for i in range(nblocks):
            if len(mol.blocks) and not len(mol.stems): break
            if i==0:
                mdp.add_block(rng.randint(853,884), rng.randint(max(1, len(mol.stems))))
            else:
                mdp.add_block(rng.randint(853), rng.randint(max(1, len(mol.stems))))
