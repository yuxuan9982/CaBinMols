...
def mol_from_frag(jun_bonds, frags=None, frag_smis=None, coord=None, optimize=False):
    ...
    for jb in jun_bonds:
        ...
        # 找到 dummy 原子的邻居
        mol_tmp = emol.GetMol()
        atom1 = mol_tmp.GetAtomWithIdx(idx1)
        atom2 = mol_tmp.GetAtomWithIdx(idx2)
        nbr1 = [n.GetIdx() for n in atom1.GetNeighbors()]
        nbr2 = [n.GetIdx() for n in atom2.GetNeighbors()]
        if not nbr1 or not nbr2:
            raise ValueError(f"Dummy atom {idx1} or {idx2} has no neighbor")

        assert len(nbr1)==1 and len(nbr2)==1

        # 加键并记录
        emol.AddBond(int(nbr1[0]), int(nbr2[0]), bond_type)
        mol_bonds.append((nbr1[0], nbr2[0], bond_type))

        remove_idx.append(idx1)
        remove_idx.append(idx2)
    # 删除 dummy 原子（注意 idx 要从大到小删）
