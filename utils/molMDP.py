...
class BlockMoleculeData:
    ...
    def add_block(self, block_idx, block, block_r, stem_idx, atmidx):
        ...
        # print(block_idx,block_r,self.stems)
        ...
        if len(self.blocks)==1:
            ...
            # destroy properties
            self._mol = None
        # print(block_idx,block_r,self.stems);input("222")
        return None
...
class MolMDP:
    def __init__(self, blocks_sub_file, blocks_core_file):
        blocks = pd.read_json(blocks_sub_file)
        ...
        self.sub_num = len(blocks["block_smi"].to_list())
        print("before add core, size is ", len(self.block_mols))

        blocks_core = pd.read_json(blocks_core_file)
        print("add len ",len(blocks_core["block_smi"].to_list()))
        self.block_smi.extend(blocks_core["block_smi"].to_list())
        self.block_rs.extend(blocks_core["block_r"].to_list())
        self.block_nrs = np.asarray([len(r) for r in self.block_rs])
        self.block_mols.extend([Chem.MolFromSmiles(smi) for smi in blocks_core["block_smi"]])
        self.block_natm = np.asarray([b.GetNumAtoms() for b in self.block_mols])
        self.core_num = len(blocks_core["block_smi"].to_list())
        print("after add core, size is ", len(self.block_mols))
        self.reset()
